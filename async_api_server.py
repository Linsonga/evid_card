...
def _mine_data_batch(task_id: str, target_batch_id: str, embeddings: np.ndarray, texts: list, metadata: list,
    ...
    for cluster_id, items in clusters_pool.items():
        ...
        5. ⚠️ 绝不能与【历史选题黑名单】在临床决策点上重复！如果你发现本次素材只能生成与黑名单高度相似的选题，请放弃生成，直接输出空数组 []。
        ...
        【本轮任务】
        输出5-10个个证据卡片候选选题清单，绝对禁止出现任何中文或英文标点符号。
        请仔细对比【历史选题黑名单】，主动拉开差异。
        请严格输出一个标准的JSON数组格式，单占一行，必须使用双引号，格式如下：["标题1", "标题2", "标题3"]。
        如果实在没有新角度，输出 []。
        """
