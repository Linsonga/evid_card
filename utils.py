...
def generate_ai_cover_dashscope_api(title, output_dir="images/covers"):
    ...
    layout_pool = [
        # 1. 居中排布
        "【居中排布构图】：采用绝对居中构图。核心视觉元素与文字在画面正中央紧凑组合（可上下或紧密左右相邻排列），画面四周必须保留大面积干净的纯色留白（负空间），突出主体。",

        # 2. 左图右文
        "【左图右文构图】：采用左右平衡构图。核心视觉元素明确安置于画面【左半部分】（垂直居中）；单句核心医学关键词排布于画面【右半部分】（垂直居中），左右遥相呼应，中间留有过渡的呼吸空间。",

        # 3. 左文右图
        "【左文右图构图】：采用左右平衡构图。单句核心医学关键词明确排布于画面【左半部分】（垂直居中）；核心视觉元素安置于画面【右半部分】（垂直居中），左右遥相呼应，中间留有过渡的呼吸空间。"
    ]

    # 在生成时，你可以随机选择或按顺序传入
    selected_layout = random.choice(layout_pool)


    # 2. 构造提示词1
    prompt = f"""你是一名医学科普插画设计师。请根据给定主题，生成一张用于AI绘图（Text-to-Image）的封面插画提示词描述。

    【输入主题】：{title}

    【通用核心要求】
    1. 视觉线索：提取 1-2 个最核心的医学关键词作为视觉主体。
    2. 元素连接：设置2个视觉元素，元素之间需有自然、符合逻辑的连接或过渡。
    3. 尺寸限制：核心医学视觉元素必须设定为【小巧、精致】（仅占画面5%-10%的比例），绝对拒绝庞大臃肿，体现专业感。
    4. 【视觉形态特征】：(← 新增这一条)
       - 医学元素必须采用现代Q版化、高纯度矢量的卡通风格设计。
       - 形态上要【圆润、简化细节】，拒绝严谨、冰冷、复杂的解剖结构描述。
       - 增加亲和力，避免视觉上的僵硬和死板。
    【本张插画的专属构图与排布设定】
    （请严格按照以下空间关系进行画面元素的排布描述）
    {selected_layout}
    - 文字限制：画面中仅允许保留一个核心医学关键词，严禁出现长句或多余的解释性文字。

    【本张插画的专属视觉风格设定】
    {selected_style}

    请输出一段画面感强、方位描述清晰的AI绘图提示词：
    """
